...
pub const CURRENT_BLOCK_HEIGHT: u64 = 951_400;
...
pub fn generate_address(prefix: &str) -> String {
    ...
    let suffix_len = 32usize.saturating_sub(prefix.len());
...
pub fn halving_schedule(blocks: &[u64]) -> HashMap<u64, u64> {
    ...
    for &block in blocks {
        ...
        // Guard against shifting by >= 64 (reward effectively 0 after 64 halvings).
        let reward = if halvings >= 64 { 0 } else { base >> halvings };
...
pub fn extract_tx_version(raw_tx_hex: &str) -> Result<u32, String> {
    // The version is the first 4 bytes (8 hex chars), stored little-endian.
